...
def register(mcp: FastMCP) -> None:
    ...
    def create_drum_pattern(track: str, pattern: str) -> dict:
        """Step-grid string → GM drum MIDI (kick 36 / snare 38 / hat 42, ch9) with humanization."""
        raise NotImplementedError("M4 — see docs/roadmap.md")

    @mcp.tool(annotations=_DESTRUCTIVE)
    def humanize_pass(track: str) -> dict:
        """±~12ms timing + ±6 velocity jitter on generated MIDI — cheap, big perceived-quality win."""
        raise NotImplementedError("M4 — see docs/roadmap.md")
