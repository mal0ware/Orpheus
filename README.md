...
> 🚧 **Status: design + scaffold (pre-alpha).** This repo currently contains the full architecture, a [documented analysis of the entire Reaper/DAW-MCP frontier](docs/frontier-analysis.md), and a complete package skeleton — **not yet a runnable tool.** The bridge to REAPER and the analysis tools are stubs being built against the [roadmap](docs/roadmap.md); `uvx orpheus-mcp` won't do anything useful yet. The music-theory core *is* implemented and tested. The first runnable release (`v0.1`) ships the **understand-and-explain** half; the **transform** half follows in `v0.3`. Star/watch to follow along. ⭐
